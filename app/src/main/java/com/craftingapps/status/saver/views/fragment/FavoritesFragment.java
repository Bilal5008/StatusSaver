...
import com.craftingapps.status.saver.databinding.FragmentFavourtiesBinding;
...
public class FavoritesFragment extends Fragment {
    ...
    FragmentFavourtiesBinding binding;

    StatusAdapter statusAdapter;
    ArrayList<StatusModel> favoritesList;


    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        binding = DataBindingUtil.inflate(inflater, R.layout.fragment_favourties, container, false);
    ...
    private void setUpRecyclerView() {
        statusAdapter = new StatusAdapter(getContext(), favoritesList);
        binding.favoritesRecycler.setHasFixedSize(true);
        binding.favoritesRecycler.setLayoutManager(new GridLayoutManager(getContext(), 2));
        binding.favoritesRecycler.setAdapter(statusAdapter);
    ...
    private ArrayList<StatusModel> getFavoritesList() {
        StatusModel status;
        String targetPath = Environment.getExternalStorageDirectory().getAbsolutePath() + AppConstants.SAVE_FOLDER_NAME;
        ...
        if (files == null) {
            ...
            try {
                ...
                for (int i = 0; i < files.length; i++) {
                    ...
                    if (files[i].getName().toString().endsWith(".jpg")) {
                        status.setVideo(false);
                    } else {
                        status.setVideo(true);
                    }
                    favoritesList.add(status);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
